...
def save_2_jsonl_with_idx(idx, question, answer, cluster_chain_of_entities, file_name, lock):
    dict = {"index": idx, "question": question, "results": answer,
...
def half_stop_with_idx(idx, question, cluster_chain_of_entities, depth, args, lock):
    ...
    save_2_jsonl_with_idx(idx, question, answer, cluster_chain_of_entities,
                          args.dataset, lock)
...
def __find_answer(worker_idx, st, ed, datas):
    ...
    for n, data in enumerate(datas):
        ...
        try:
            ...
            if len(topic_entity) == 0:
                ...
                save_2_jsonl_with_idx(idx, question, results,
                                      [], args.dataset, lock)
                continue
            pre_relations = []
            ...
            for depth in range(1, args.depth+1):
                ...
                for entity in topic_entity:
                    ...
                    if entity != "[FINISH_ID]":
                        retrieve_relations_with_scores = relation_search_prune(
                            entity, topic_entity[entity], pre_relations, pre_heads[i], question, args)  # best entity triplet, entitiy_id
                ...
                if len(total_candidates) == 0:
                    half_stop_with_idx(
                        idx, question, cluster_chain_of_entities, depth, args, lock)
                    flag_printed = True
                    break
                ...
                if flag:
                    ...
                    if stop:
                        ...
                        save_2_jsonl_with_idx(idx, question, results,
                                              cluster_chain_of_entities, args.dataset, lock)
                        flag_printed = True
                        break
                    else:
...
if __name__ == '__main__':
    ...
    if os.path.exists("ToG_{}.jsonl".format(args.dataset)):
        with open("ToG_{}.jsonl".format(args.dataset), 'r') as f:
            output_datas = [json.loads(line) for line in f.readlines()]
        processed_idx = set([data['index'] for data in output_datas])
        datas = [data for data in datas if data['index'] not in processed_idx]
    ...
    with Pool(processes=n_process) as pool:
        ...
        for i in range(n_process):
            ...
            jobs.append([i, st, ed, datas[st:ed]])

            st = ed

        results = pool.starmap(__find_answer, jobs)

    convert_jsonl_to_json("ToG_{}.jsonl".format(args.dataset))